# Litentry

![litentry](media/avatar.png)


## 📚 Documentation

### Core Documents

- 📜 [Constitutional Research](CONSTITUTIONAL_RESEARCH.md) - Core project overview and privacy features
- ℹ️ [Project Metadata](PROJECT_METADATA.md) - Basic project information and categorization

### Analysis

- 📝 [Smart Contracts](analysis/SMART_CONTRACTS.md) - Contract analysis and code review
- 💻 [Code Analysis](analysis/CODE_ANALYSIS.md) - Technical code review and metrics
- 📊 [GitHub Analysis](analysis/GITHUB_ANALYSIS.md) - Repository statistics and activity

### Reports

- 📰 [News & Updates](reports/NEWS_SUMMARY.md) - Recent articles and developments

---


## 📝 Description

Litentry Foundation’s vision is to build a decentralized, privacy-preserving, and interoperable Web3 ecosystem where users have full control over their digital identities and data.

## 🔗 Links

- **Website**: https://litentry.network
- **GitHub**: https://github.com/litentry/litentry-parachain

## 🏷️ Category

Privacy Technology

## 💻 Code Analysis

- **Total Lines of Code**: 262,826
- **Primary Language**: Rust
- **Smart Contracts**: 70 Solidity files
- **Test Coverage**: 343 test files

## 📚 Research Files

This project includes comprehensive research data:

### Reports
- [News Report](reports/news_report.md)
- [Oso Assessment 2025 09 29](reports/oso_assessment_2025-09-29.md)

### Analysis Data
- [Code Analysis](analysis/code_analysis.json)
- [Github Analysis](analysis/github_analysis.json)
- [Oso Data](analysis/oso_data.json)
- [Smart Contracts](analysis/smart_contracts.json)

### Raw Research Data
- [Research Methodology JSON](constitutional_research.json) - Complete structured data
- [Sources](sources/) - Raw research sources and references

---

*Research Methodology: 100% Real Data - Zero Fabrication*
